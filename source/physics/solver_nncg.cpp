...
void SolverNNCG::solve(float dt)
{
	if (dt < 0.00001f)
	{
		return;
	...
	float * ptr_m_dLamVec = &m_dLamVec[0];
	float * ptr_m_p = &m_p[0];
	...
	// First PGS pre-iteration
	{
		ptr_m_dLamVec = &m_dLamVec[0];
		ptr_m_p = &m_p[0];
		...
		for (i = 0; i < m_numJoints; ++i)
		{
			...
			ptr_m_dLamVec[i] = -dLam;
			ptr_m_p[i] = -ptr_m_dLamVec[i];
			dLamNorm += dLam*dLam;
	...
	for (unsigned k = 0; k < m_maxIterations; ++k)
	{
		...
		if (dLamNormPrev != 0.0f)
			beta = dLamNorm / dLamNormPrev;
		...
		else
		{
			...
			for (i = 0; i < m_numJoints; ++i)
			{
				float dLamNNCG = beta * ptr_m_p[i];
				ptr_m_lambda[i] = ptr_m_lambda[i] + dLamNNCG;
				ptr_m_p[i] = beta * ptr_m_p[i] - ptr_m_dLamVec[i];

				// Update a = Asp * lambda = dt * M^{-1} * J^T * lambda
				//	using new values of lambda
				...
				ptr_m_a_x[t0Idx] += inc_arr(m_Asp_00) * dLamNNCG;
				ptr_m_a_y[t0Idx] += inc_arr(m_Asp_01) * dLamNNCG;
				ptr_m_a_z[t0Idx] += inc_arr(m_Asp_02) * dLamNNCG;
				ptr_m_a_x[t1Idx] += inc_arr(m_Asp_03) * dLamNNCG;
				ptr_m_a_y[t1Idx] += inc_arr(m_Asp_04) * dLamNNCG;
				ptr_m_a_z[t1Idx] += inc_arr(m_Asp_05) * dLamNNCG;
				if (t2Idx != SOLVER_NO_BODY)
				{
					ptr_m_a_x[t2Idx] += val_arr(m_Asp_06) * dLamNNCG;
					ptr_m_a_y[t2Idx] += val_arr(m_Asp_07) * dLamNNCG;
					ptr_m_a_z[t2Idx] += val_arr(m_Asp_08) * dLamNNCG;
				}
				if (t3Idx != SOLVER_NO_BODY)
				{
					ptr_m_a_x[t3Idx] += val_arr(m_Asp_09) * dLamNNCG;
					ptr_m_a_y[t3Idx] += val_arr(m_Asp_10) * dLamNNCG;
					ptr_m_a_z[t3Idx] += val_arr(m_Asp_11) * dLamNNCG;
				}
				inc_arr(m_Asp_06);
				inc_arr(m_Asp_07);
				inc_arr(m_Asp_08);
				...
				if (t4Idx != SOLVER_NO_BODY)
				{
					ptr_m_a_x[t4Idx] += val_arr(m_Asp_12) * dLamNNCG;
					ptr_m_a_y[t4Idx] += val_arr(m_Asp_13) * dLamNNCG;
					ptr_m_a_z[t4Idx] += val_arr(m_Asp_14) * dLamNNCG;
				}
				if (t5Idx != SOLVER_NO_BODY)
				{
					ptr_m_a_x[t5Idx] += val_arr(m_Asp_15) * dLamNNCG;
					ptr_m_a_y[t5Idx] += val_arr(m_Asp_16) * dLamNNCG;
					ptr_m_a_z[t5Idx] += val_arr(m_Asp_17) * dLamNNCG;
				}
				inc_arr(m_Asp_12);
				inc_arr(m_Asp_13);
				inc_arr(m_Asp_14);
